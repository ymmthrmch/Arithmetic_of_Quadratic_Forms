...
\end{proof}
\subsection{反転代数}
さらに中心的単純代数の性質を調べていくために反転代数を導入しよう．行列の転置や，次章で扱うハミルトンの四元数代数の共役は，加法やスカラー倍は保存するが
\[
...
\begin{proof}
  ...
  \]
\end{proof}
一般の代数に対する分離性の定義はこの補題\ref{lem:csa_coef_ext_is_ssa}が成り立つこと，つまり$A\otimes_F L$が半単純であることと定義される．

例\ref{eg:tr_and_n_of_matrix}では$\gr={\id_A}^{\oplus n}$であり，これがトレース，ノルムよりも$\m_n(F)$の通常のトレース，行列式の方が精密に情報を取り出せる理由だった．一般の分離単純代数でもこのように右正則表現は別のものの直和に分解できる．
...
\end{eg}
被約トレースの性質を見てみよう．
\begin{thm}[分離SAの被約トレースは非退化]
  $A$を分離単純$F$代数とする．$F$双線型形式$(x,y)\mapsto\trd_{A/F}(xy)$は非退化である．
\end{thm}
\begin{proof}

\end{proof}
今後使う補題を示しておく．これは補題\ref{lem:csa_coef_ext_is_ssa}の部分的な逆である．
