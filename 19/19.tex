...
\begin{thm}[CSAの次元は平方数]
  $A$が中心的単純$F$代数ならば$[A:F]$は平方数である．
\end{thm}
\begin{proof}
  定理\ref{thm:csa_judgement}(2)よりしたがう．
\end{proof}
...
\begin{thm}[CSAのSA部分代数は共役]\label{thm:sub_sa_are_conj}
...
\begin{proof}
  ...
  さらに
  \[
    [W:F]=[D:F]rs,\quad [A:F]=[D:F]r^2,\quad [\End_A(W):F]=[D:F]s^2
  \]
  ゆえ$[W:F]^2=[A:F][\End_A(W):F]$が成り立つ．
\end{proof}
\begin{thm}[CSAでのSAの中心化代数]
...
\end{thm}
\begin{lem}[CDAは$F$の分離拡大を含む]

\end{lem}
\begin{thm}[CDAが含む分離拡大の次数]
