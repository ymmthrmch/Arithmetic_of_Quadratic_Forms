...
\begin{prop}[中心の性質]\label{prop:prop_of_center}
  ...
  \begin{enumerate}
    \item $\Zen\left(\bigoplus_{\gl\in\gL}A_{\gl}\right)=\bigoplus_{\gl\in\gL}\Zen(A_{\gl}).$
    \item $\Zen\left(\m_r(A)\right)=\Zen(A)I_r.$
    \item $D$が可除環のとき$\Zen(D)$は体である．
...
\begin{dfn}[$F$代数のテンソル積]
  ...
  \]
  が第一，二引数に対して$F$線形になるように拡張して定める．これはwell-definedであり，この積により$A\otimes B$は$F$代数となる．
\end{dfn}
\begin{proof}
...
\end{proof}
\subsection{反転代数}
さらに中心的単純代数の性質を調べていくために反転代数を導入しよう．行列の転置や，次節で扱うハミルトンの四元数代数の共役は，加法やスカラー倍は保存するが
\[
...
\end{proof}
\subsection{被約トレース・被約ノルム}
$F$代数に対してトレースやノルムといった概念を定めよう．まずトレースとノルムを定義する際に用いる表現を導入する．
\begin{dfn}[表現]
  \begin{enumerate}
    \item $F$代数の$A$の次数$r$の表現とは，$F$準同型$\gs:A\to\m_r(F)$のことである．
    \item $A$の表現$\gs$が忠実であるとは，$\gs$が単射であることである．
    \item $A$の表現$\gs,\gt$が同値であるとは，次数が等しく（$r$とおく），ある$\gg\in\GL_r(F)$が存在し，任意の$a\in A$に対して
          \[
            a^\gt = \gg a^\gs \gg^{-1}
          \]
          が成り立つことであり，$\gt\sim\gs$と書く．
    \item $A$の右正則表現$\gr$とは，$A$から右$A$加群$A_A$への作用を与える準同型$\gr:A\to\End_F(A_A)\cong \m_{[A:F]}(F)$のことである．これは同値を除いて定まる．
    \item 次数が等しい$F$代数$A$の表現$\gs,\gt$について，その直和$\gs\oplus\gt$とは
          \[
            a^{\gs\oplus\gt}=\mqty(a^\gs &O\\O& a^\gt)
          \]
          により定まる表現のことである．
  \end{enumerate}
\end{dfn}
$A$が$F$ベクトル空間$V$に作用している時，$V$の基底をとるごとに次数$\dim V$の表現が定まる．このときの基底の取り方の違いを無視する同一視を行うのが表現の同値である．逆に表現も$F$ベクトル空間への作用を誘導するため加群を誘導する．表現の同値性は加群を使うと以下のように言い換えられる．
\begin{prop}[同値な表現の加群は同型]\label{prop:equiv_and_iso}
  $A$を$F$代数，$\gs,\gt$を$A$の次数$r$の表現とする．$\gs,\gt$が誘導する作用により$V=F^r$を$A$加群とみなしたものをそれぞれ$V_{\gs},V_{\gt}$とおく．このとき
  \[
    \gs\sim\gt \iff V_{\gs}\cong V_{\gt}
  \]
  である．
\end{prop}
\begin{proof}
  $V_\gs\cong V_\gt$が同型であるとは$\gg\in \GL_r(F)$が存在し，任意の$a\in A,v\in F^r$に対して
  \[
    (v a^{\gt})\gg=(v\gg)a^{\gs}
  \]
  ...
  と同値である．
\end{proof}
有限次拡大体$K/F$の場合はトレース，ノルムを右正則表現$\gr$を用いて
\[
  \tr_{K/F}(a)=\tr(a^\gr),\quad \n_{K/F}(a)=\det(a^\gr)
\]
と定めた．つまり各元をスカラー倍により線型写像とみなし、その線型写像のトレースやノルムをその元のトレース、ノルムにするということである．これと同じように$F$代数$A$にトレース、ノルムを定めることができる．
\begin{dfn}[トレース・ノルム]
  $A$を$F$代数とする．$A$の右正則表現$\gr$を用いて
  \[
    \tr_{A/F}(a)=\tr(a^\gr),\quad \n_{A/F}(a)=\det(a^\gr).
  \]
  で定められる写像$\tr_{A/F}:A\to F,\n_{A/F}:A\to F$をそれぞれ$A$のトレース，ノルムという．
\end{dfn}
しかし場合によってはこのトレース，ノルムが与える情報はある意味で少なくなっている．例を見てみよう．
\begin{eg}\label{eg:tr_and_n_of_matrix}
  $A=\m_n(F)$のとき，標準的な基底$\{E_{1,1},\ldots,E_{1,n},\ldots,E_{n,1},\ldots,E_{n,n}\}$（$E_{i,j}$は$(i,j)$成分が1で他は0の$n\times n$行列）に関する$a\in A$の右正則表現は
  \[
    a^\gr=\mqty(
    a&&\\
    &\ddots&\\
  ...
  \end{align*}
  となる．$\det(a)\neq \det(b)$だが$\n_{A/F}(a)=\n_{A/F}(b)$なる$a,b$が（もし$F$が$a^n-1=0$の非自明な解を持つなら）存在する．また標数が$n$を割り切るとき$\tr_{A/F}$は常に$0$になる．これらの意味で，$\n_{A/F},\tr_{A/F}$は$\m_r(F)$の通常の行列式，トレースよりも情報量が少ない．
\end{eg}
できればこの例の場合の通常の行列式やトレースのような，$\n_{A/F},\tr_{A/F}$を簡約したものを利用したい．そこでこのようなものを被約ノルム，被約トレースとして定めることを目指す．
被約ノルム，被約トレースを定めるために代数に課す性質を表す言葉を用意しておく．
\begin{dfn}[分離単純代数]
  $F$代数$A$が分離単純$F$代数であるとは，$A$が単純$F$代数であり，$\Zen(A)/F$が分離拡大であることである．
\end{dfn}
分離性自体は一般の代数に対して定義されるが，ここでは簡潔さのために単純代数に限定して上記の定義を採用する．いくつか定理，補題を示す．
\begin{thm}[SAの表現
    %,SSAの正則表現の同値性
  ]\label{thm:sa_rep_and_ssa_reg_rep}
  % \begin{enumerate}
  %   \item 
  単純$F$代数の次数が等しい二つの表現は同値である．
...
\begin{proof}
  % \begin{enumerate}
  %   \item 
  補題\ref{lem:same_dim}と命題\ref{prop:equiv_and_iso}よりしたがう．
...
\begin{lem}[分離SAの係数拡大はSSA]\label{lem:csa_coef_ext_is_ssa}
  $A$を分離単純$F$代数とする．$F$の（有限次元とは限らない）任意の拡大体$L$について，$A\otimes_F L$は半単純$F$代数である．
\end{lem}
\begin{proof}
  $Z=\Zen(A)$とおく．$\gW$を$L$のガロア拡大体で$Z$の$F$準同型像を全て含むものとする．
  \[
    S=\hom_F(Z,\gW)/\gal(\gW/L),\quad U_\gs=Z^\gs L
  \]
  とおく．$Z/F$が分離拡大であることから定理\ref{thm:tensor_is_a_direct_sum_of_inj_images}より
  \[
  ...
  \]
  ただし$Z\ni z\mapsto z^\gs \in U_\gs$により$Z\subseteq U_\gs$とみなす．$U_\gs$は$Z$の拡大体ゆえ単純$Z$代数である．定理\ref{thm:csa_tensor_preserves_sa_ssa_csa}より$A\otimes_Z U_\gs$は単純$U_\gs$代数である．したがって，$A\otimes_F L$は単純な部分代数の直和であるから半単純である．
\end{proof}
一般の代数に対する分離性の定義はこの補題\ref{lem:csa_coef_ext_is_ssa}が成り立つこと，つまり$A\otimes_F L$が半単純であることと定義される．この補題\ref{lem:csa_coef_ext_is_ssa}は部分的に逆が成立する．
\begin{lem}[中心が分離拡大で係数拡大してSSAなら分離SA]
  $A$を$\Zen(A)$が$F$の分離拡大体であるような$F$代数とする．$F$のある拡大体$L$について，$A\otimes_F L$が半単純であるとき，$A$は分離単純である．
\end{lem}
\begin{proof}
  $Z=\Zen(A)$とおく．補題\ref{lem:csa_coef_ext_is_ssa}の証明と同じ記号，論証で
  \[
    A\otimes_F L\cong \bigoplus_{\gs\in S}\left(A\otimes_Z U_{\gs}\right)
  \]
  が得られる（この分解は$A$の単純性を仮定していない）．
  $A\otimes_F L$は半単純代数ゆえ命題\ref{prop:semisimple}より右${A\otimes_F L}$加群$(A\otimes_F L)_{A\otimes_F L}$は半単純加群である．各$A\otimes_Z U_\gs$はこの直和因子であるから命題\ref{prop:complete_summand}(2)より半単純右$A\otimes_F L$加群である．$\tau\neq\gs$の項$A\otimes_Z U_\tau$の$A\otimes_Z U_\gs$への作用は零であるから，$A\otimes_Z U_\gs$の右$A\otimes_F L$加群構造は右$A\otimes_Z U_\gs$加群構造に一致する．再び命題\ref{prop:semisimple}より$A\otimes_Z U_\gs$は半単純代数である．補題\ref{lem:tensor_and_center}(2)を$Z$代数$A$とその係数拡大$A\otimes_Z U_\gs$に対して適用すると，
  \[
    \Zen(A\otimes_Z U_{\gs})=\Zen(A)\otimes_Z U_{\gs}=Z\otimes_Z U_{\gs}\cong U_{\gs}
  \]
  であるから$\Zen(A\otimes_Z U_{\gs})$は体である．よって命題\ref{prop:prop_of_center}(7)より，$A\otimes_Z U_\gs$は単純である．したがって$A\otimes_Z U_\gs$は中心的単純$U_\gs$代数である．

  定理\ref{thm:csa_judgement}(1)を$Z$代数$A$とその係数拡大$A\otimes_Z U_\gs$に適用すると，$A$は中心的単純$Z$代数である．特に$A$は単純である．これと仮定より$\Zen(A)/F$が分離拡大であるから，$A$は分離単純$F$代数である．
\end{proof}
例\ref{eg:tr_and_n_of_matrix}では$\gr={\id_A}^{\oplus n}$であり，これがトレース，ノルムよりも$\m_n(F)$の通常のトレース，行列式の方が精密に情報を取り出せる理由だった．一般の分離単純代数でもこのように右正則表現は別のものの直和に分解できる．このことが被約トレース，被約ノルムを定めるのに重要になる．
\begin{thm}[分離SAの正則表現の分解]\label{thm:sep_sa_reg_rep_decomp}
  $A$を分離単純$F$代数とする．$[A:\Zen(A)]=n^2, [\Zen(A):F]=m$とおく．$A$の右正則表現$\gr:A\to\m_{n^2m}(F)(\into\m_{n^2m}(\overline{F}))$に対して，ある$F$準同型$\gt:A\to\m_{mn}(\overline{F})$が存在し，
  \[
    \gr\sim\gt^{\oplus n}
  \]
  が成り立つ．さらに$\gch(P)(x)$で行列$P$の特性多項式を表すと，任意の$a\in A$に対して
  \[
    \gch(a^\gt)(x)\in F[x],\quad\gch(a^\gr)(x)=\gch(a^\gt)(x)^n
  \]
  が成り立つ．
\end{thm}
\begin{proof}
  ...
  である．したがって，
  \begin{equation}
    A\otimes_F L \cong \bigoplus_{\gs\in S} \m_{n}(U_{\gs})\label{eq:sep_sa_coef_ext_is_a_direct_sum_of_matrix_algebras}
  \end{equation}
  である．ここで$rs=n$を用いた．\\
  次に$\gr$の$A\otimes_F L$への延長$\gr_L:A\otimes_F L\to\m_{n^2m}(L)$について調べる．これは$L$代数$A\otimes_F L$の右正則表現である．$\gr_\gs:\m_n(U_\gs)\to\m_{n^2}(L)$を$L$代数$\m_n(U_\gs)$の右正則表現とする．$A\otimes_F L\cong\bigoplus_{\gs\in S} \m_{n}(U_{\gs})$ゆえこの同型を通じて$\gr_L=\bigoplus_{\gs\in S}\gr_\gs$である．各$\gr_\gs$について，$\m_n(U_\gs)$は単純$L$代数ゆえ定理\ref{thm:sa_rep_and_ssa_reg_rep}(1)より$\gr_\gs\sim {\id_{\m_r(U_\gs)}}^{\oplus n}$である．したがって$\gt=\bigoplus_{\gs\in S}\id_{\m_r(U_\gs)}$とおくと
  \[
    \gr_L=\bigoplus_{\gs\in S}\gr_\gs\sim\bigoplus_{\gs\in S} {\id_{\m_r(U_\gs)}}^{\oplus n}=\left(\bigoplus_{\gs\in S}\id_{\m_r(U_\gs)}\right)^{\oplus n}=\gt^{\oplus n}
  \]
  である．$\gr=\gr_L|_A$であるため，$\gr,\gt$の定義域，余域を適切に読み直して$\gr\sim \gt^{\oplus n}$が成り立つ．\\
  最後に$\gch(\gt(a))(x)$について調べる．まず
  \begin{align*}
    \gch(a^\gt)(x)^n & =\det(xI_{nm} - a^\gt)^n          \\
                     & =\det\mqty(xI_{nm} - a^{\gt} &  & \\&\ddots&\\&&xI_{nm}-a^\gt)\\
                     & =\det(xI_{n^2m} - a^\gr)          \\
                     & =\gch(a^\gr)(x)
  \end{align*}
  より$\gch(a^\gt)(x)^n=\gch(a^\gr)(x)$が成り立つ．次に$\gch(a^\gt)(x)\in F[x]$を示す．$\gch(a^\gt)(x)\in L[x]$であるので$\gal(L/F)$の作用で不変であることを示せば良い．$\gch(a^\gr)(x)\in F[x]$より任意の$\gs\in \gal(L/F)$について
  \[
    (\gch(a^\gt)(x))^\gs)^n=(\gch(a^\gr)(x))^\gs=\gch(a^\gr)(x)=\gch(a^\gt)(x)^n
  \]
  である．よって$\gz$を1の原始$n$乗根として$\gch(a^\gt)(x)^\gs=\gz^k\gch(a^\gt)(x)$が成り立つが，どちらもモニックなので$k=1$，すなわち$\gch(a^\gt)(x)^\gs=\gch(a^\gt)(x)$である．
\end{proof}
この定理を用いて分離単純$F$代数の被約トレース，被約ノルムを定義しよう．右正則表現を分解して得られた表現を用いて定める．
\begin{dfn}[被約ノルム，被約トレース]
  $A$を分離単純$F$代数，$[A:\Zen(A)]=n^2, [\Zen(A):F]=m$，$\gr$を$A$の右正則表現とする．$\gt:A\to\m_{mn}(\overline{F})$を$\gr\sim \gt^{\oplus n}$なる表現とする．このとき
  \[
    \trd_{A/F}(a)=\tr(a^\gt),\quad\nrd_{A/F}(a)=\det(a^\gt)
  \]
  で定められる写像$\trd_{A/F}:A\to F,\nrd_{A/F}:A\to F$をそれぞれ$A$の被約トレース，被約ノルムという．
\end{dfn}
定理\ref{thm:sep_sa_reg_rep_decomp}によりこのような$\gt$は存在する．$\gt$の選択でのwell-defined性を示す．
\begin{proof}
  $\gr\sim\gt^{\oplus n}\sim\gs^{\oplus n}$であるとき，$\gg\gt^{\oplus n}=\gs^{\oplus n}\gg$なる$\gg\in\GL_{mn}(\overline{F})$が存在する．$\gg$を$n\times n$のブロック行列とみなすと，$\gg\gt^{\oplus n}=\gs^{\oplus n}\gg$より任意の$i,j\in\{1,\ldots,n\}$について$\gg_{ij}a^\gt=a^\gs\gg_{ij}$が成り立つ．$\gg$が正則ゆえ$\gg_{ij}$に正則なものが存在する．したがって$a^\gt$と$a^\gs$は相似である．よって$\trd_{A/F}(a)$と$\nrd_{A/F}(a)$は$\gt$の選択によらずwell-definedである．
\end{proof}
いくつか簡単な例を見ておこう．
\begin{eg}
  $A=\m_n(F)$のとき，$[A:\Zen(A)]=n^2,[\Zen(A):F]=1$である．$a\in A$について，
  \[
    a^\gr=\mqty(
    a&&\\
    &\ddots&\\
    ...
    =a^{{\id_A}^{\oplus n}},
  \]
  つまり$\gr\sim{\id_A}^{\oplus n}$である．したがって
  \[
    \trd_{A/F}(a)=\tr(a),\quad\nrd_{A/F}(a)=\det(a)
  \]
  である．例\ref{eg:tr_and_n_of_matrix}の観察を通じて望んだ通り，トレース，ノルムを簡約したものとして被約トレース，被約ノルムが得られていることがわかる．
\end{eg}
\begin{eg}
  $K/F$を分離拡大体とする．$A=K$のとき，$[A:\Zen(A)]=1,[\Zen(A):F]=[K:F]$である．$\gr\sim\gr^{\oplus 1}$である．したがって
  \[
    \trd_{A/F}(a)=\tr_{K/F}(a),\quad\nrd_{A/F}(a)=\n_{K/F}(a)
  \]
  である．この例では被約トレース，被約ノルムは通常のトレース，ノルムと一致している．
\end{eg}
被約トレースの性質を見てみよう．
\begin{thm}[分離SAの被約トレース形式は非退化]
  $A$を分離単純$F$代数とする．（被約）トレース形式と呼ばれる$F$双線型形式
  \[
    A\times A\ni (a,b)\mapsto\trd_{A/F}(ab)\in F
  \]
  は非退化である．
\end{thm}
\begin{proof}
  定理\ref{thm:sep_sa_reg_rep_decomp}の証明と同様に$Z,L,S,U_\gs,\gt$を定める．トレース形式を$A\otimes_F L$に延長した$L$双線型形式が非退化なことを示せば十分である．式(\ref{eq:sep_sa_coef_ext_is_a_direct_sum_of_matrix_algebras})の同型$A\otimes_F L\cong \bigoplus_{\gs\in S} \m_n(L)$で両者を同一視する．定理\ref{thm:sep_sa_reg_rep_decomp}の証明での$\gt$の定義と被約トレースの定義を振り返ると，被約トレースの延長は
  \[
    \bigoplus_{\gs\in S} \m_n(L)\ni (a_\gs)_\gs\mapsto \sum_{\gs\in S}\tr(a_\gs)\in L
  \]
  である．つまり$A$のトレース形式の$A\otimes_F L$への延長は$\m_n(L)$のトレース形式の$\# S$個の直和になっている．$\m_n(L)$のトレース形式は非退化であるから，$A\otimes_F L$のトレース形式も非退化である．
\end{proof}
%%%%%%%%%%%%%%%%%%%%%content%%%%%%%%%%%%%%%%%%%%%
